...
def wait_for_not_found(
    stub, navigator_pb2, sandbox_id: str, timeout: float = 60.0
) -> None:
    deadline = time.time() + timeout
    while time.time() < deadline:
        try:
            stub.GetSandbox(navigator_pb2.GetSandboxRequest(id=sandbox_id))
...
def test_sandbox_grpc_crud() -> None:
    ...
    get_resp = stub.GetSandbox(navigator_pb2.GetSandboxRequest(id=sandbox.id))
    ...
    delete_resp = stub.DeleteSandbox(navigator_pb2.DeleteSandboxRequest(id=sandbox.id))
    assert delete_resp.deleted is True

    wait_for_not_found(stub, navigator_pb2, sandbox.id)
